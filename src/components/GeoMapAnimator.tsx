...
import { Map as MapIcon, UploadCloud, Play, Pause, Navigation, MapPin, Loader2, Calendar, Download, FileSpreadsheet, Globe } from 'lucide-react';
import Papa from 'papaparse';
import * as XLSX from 'xlsx';
import { cn } from './Layout';
import { MapContainer, TileLayer, Marker, Polyline, Popup, Tooltip, useMap, CircleMarker, LayersControl, LayerGroup } from 'react-leaflet';
...
function getVehicleType(prev: LocationData | undefined | null, curr: LocationData | undefined | null) {
...
function getVehicleColor(type: string) {
  return '#22c55e'; // Always green as requested by user
}
...
export default function GeoMapAnimator() {
  ...
  const [fileDateBounds, setFileDateBounds] = useState<{min: number, max: number} | null>(null);
  ...
  const [interpolatedPos, setInterpolatedPos] = useState<[number, number] | null>(null);
  ...
  const lookupAddress = useCallback(async (lat: number, lng: number) => {
    ...
    try {
        ...
        const address = (data && data.display_name) ? data.display_name : 'पता नहीं मिला (Address not found)';
        setAddressCache(prev => ({ ...prev, [key]: address }));
        pendingLookups.current.delete(key);
        return address;
    } catch (e) {
        pendingLookups.current.delete(key);
        return 'पता लोड नहीं हो सका (Error loading address)';
    }
  }, [addressCache]);
  ...
  const activeVehicleType = useMemo(() => {
    if (currentIndex <= 0 || !locations[currentIndex] || !locations[currentIndex-1]) return 'car';
  ...
  const airplaneIcon = useMemo(() => new L.DivIcon({
      ...
      className: 'custom-vehicle-icon', iconSize: [40, 40], iconAnchor: [20, 20]
  }), []);
  const trainIcon = useMemo(() => new L.DivIcon({
  ...
  const carIcon = useMemo(() => new L.DivIcon({
      html: `<div style="font-size: 24px; background: white; border-radius: 50%; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1); border: 2px solid #1e293b;">🚗</div>`,
      className: 'custom-vehicle-icon', iconSize: [40, 40], iconAnchor: [20, 20]
  }), []);
  ...
  const walkIcon = useMemo(() => new L.DivIcon({
      html: `<div style="font-size: 24px; background: white; border-radius: 50%; width: 40px; height: 40px; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1); border: 2px solid #1e293b;">🚶</div>`,
      className: 'custom-vehicle-icon', iconSize: [40, 40], iconAnchor: [20, 20]
  }), []);
  const activeIcon = activeVehicleType === 'airplane' ? airplaneIcon :
                     activeVehicleType === 'train' ? trainIcon :
                     activeVehicleType === 'car' ? carIcon : 
                     activeVehicleType === 'bike' ? bikeIcon : walkIcon;

  useEffect(() => {
    if (animating && currentIndex > 0 && locations[currentIndex] && locations[currentIndex - 1]) {
      ...
      const animate = (time: number) => {
          ...
          if (progress < 1) {
            ...
            while (nextIdx < locations.length) {
               if (getDistance(curr.lat, curr.lng, locations[nextIdx].lat, locations[nextIdx].lng) > 0.00001) {
                  break;
               }
               nextIdx++;
            }
  ...
  const polylinePath = useMemo(() => {
     ...
     if (showFullPath) return locations.map(l => [l.lat, l.lng] as [number, number]);
  ...
  useEffect(() => {
     ...
     setCurrentAddress('लोकेशन का पता खोजा जा रहा है... (Finding address...)');
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (selected.name.endsWith('.csv') || selected.name.endsWith('.txt')) {
        ...
        parsedData = XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: '', raw: false }) as any[][];
      ...
      setTimeout(async () => {
        ...
        let headerRowIdx = -1;
        let latIdx = -1, lngIdx = -1, combinedCgiIdx = -1, dateIdx = -1, timeIdx = -1, locIdx = -1;
        ...
        for (let r = 0; r < Math.min(parsedData.length, 50); r++) {
           ...
           let score = 0;
           let tLat = -1, tLng = -1, tCgi = -1, tDate = -1, tTime = -1, tLoc = -1;

           for (let c = 0; c < row.length; c++) {
              const val = String(row[c]).toLowerCase().trim();
              if (!val) continue;

              const isLocationWord = (val.includes('location') || val.includes('address') || val.includes('place') || val.includes('पता') || val.includes('स्थान') || val.includes('site') || val.includes('area') || val.includes('city') || val.includes('vill') || val.includes('gao') || val.includes('gaon') || val.includes('tower') || val.includes('add') || val.includes('addres'));

              if (!val.includes('last') && (val.includes('lat') || val === 'lt') && tLat === -1) { tLat = c; score += 10; }
              else if (!val.includes('last') && (val.includes('lon') || val.includes('lng') || val === 'lg') && tLng === -1) { tLng = c; score += 10; }
              else if (!val.includes('last') && isLocationWord && tLoc === -1) { tLoc = c; score += 10; }
              else if (!val.includes('last') && (val.includes('cgi') || val.includes('first')) && tCgi === -1) { tCgi = c; score += 5; }
              else if (!val.includes('last') && (val.includes('date') || val.includes('dina') || val.includes('dinank') || val.includes('दिनांक')) && tDate === -1) { tDate = c; score += 2; }
              else if (!val.includes('last') && !val.includes('update') && (val.includes('time') || val.includes('samay') || val.includes('smay') || val.includes('समय')) && tTime === -1) { tTime = c; score += 2; }
           }
        ...
        if (latIdx === -1 && lngIdx === -1 && combinedCgiIdx === -1 && locIdx === -1) {
            let bestTextCol = -1;
            const colsCount = (parsedData[0] && parsedData[0].length) || 0;
            for (let c = 0; c < colsCount; c++) {
                let textScore = 0;
                for (let r = (headerRowIdx !== -1 ? headerRowIdx + 1 : 0); r < Math.min(parsedData.length, 20); r++) {
                    const row = parsedData[r];
                    if (!row || !Array.isArray(row)) continue;
                    const val = String(row[c] || '').trim();
                    if (val.length > 5 && isNaN(Number(val)) && !val.match(/^(\d{1,4})[\.\-\/](\d{1,2})/) && !val.match(/^\d{1,2}:\d{1,2}/)) {
                        textScore++;
                    }
                }
                if (textScore >= 3) {
        ...
        const parseDateTimeStr = (dStr: string, tStr: string) => {
           ...
           const dmvMatch = dStr.match(/^(\d{1,2})[\.\-\/](\d{1,2})[\.\-\/](\d{2,4})/);
           if (dmvMatch) {
              d = dmvMatch[1];
              m = dmvMatch[2];
              y = dmvMatch[3];
        ...
        const rawExtracted: {lat: number|null, lng: number|null, datetime: string, _parsedDate: number|undefined, addressString: string|null}[] = [];
        ...
        for (let r = startRow; r < parsedData.length; r++) {
            ...
            if (latIdx !== -1 && lngIdx !== -1) {
               ...
               const match = val.match(/(-?\d{1,3}\.\d{2,})[\s,;|:\/]+(-?\d{1,3}\.\d{2,})/);
               if (match) {
            ...
            if ((lat === null || lng === null || isNaN(lat) || isNaN(lng)) && locIdx !== -1) {
        ...
        const updateMapLocations = (currentRaw: any[]) => {
             ...
             if(deduplicated.length > 0) {
                 ...
                 const validLocations = deduplicated.filter(l => l.datetime && l.datetime !== '-');
        ...
        if (geocodeCount > 0) {
           ...
           for (let i = 0; i < rawExtracted.length; i++) {
              ...
              if (item.lat === null && item.addressString) {
                  ...
                  setLoadingText(`लोकेशन का पता लगाया जा रहा है\n(Geocoding ${currentGeocode} of ${geocodeCount})...\n*Rate limited to 1 address per second for free services*`);
                  try {
                      let q = item.addressString;
                      let res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&limit=1&email=test@example.com`);
                      let data = await res.json();
                      
                      if ((!data || data.length === 0) && !q.toLowerCase().includes('india')) {
                          await new Promise(r => setTimeout(r, 1100)); // wait before fallback
                          res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q + ', India')}&limit=1&email=test@example.com`);
                          data = await res.json();
                      }
        ...
        if (rawExtracted.filter(x => x.lat !== null).length === 0) {
            alert('Could not find valid Latitude/Longitude or Address points in this file.');
        }
  ...
  const toggleAnimation = () => {
    ...
    if (!animating) {
      if (currentIndex >= locations.length - 1 || currentIndex < 0) {
        ...
        if (currentIndex > 0) {
          setInterpolatedPos([locations[currentIndex-1].lat, locations[currentIndex-1].lng]);
        }
      }
      setAnimating(true);
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (dateRange.start !== null && dateRange.end !== null) {
          const sLabel = availableDates.find(d => d.index === dateRange.start)?.label || 'Start';
          const eLabel = availableDates.find(d => d.index === dateRange.end)?.label || 'End';
          doc.text(`Filtered by Date Range: ${sLabel} to ${eLabel}`, 14, currentY);
      } else {
      ...
      if (mapElement) {
         try {
           ...
           const canvas = await html2canvas(mapElement, { useCORS: true, allowTaint: true, logging: false });
           ...
           doc.addImage(imgData, 'PNG', xOffset, currentY, finalWidth, finalHeight);
           currentY += finalHeight + 15;
         } catch (e) {
           console.warn("Could not capture map image due to CORS. Continuing without map image.");
         }
      }
  ...
  const generateExcel = () => {
    if (visibleLocations.length === 0) return;
    try {
        const worksheet = XLSX.utils.json_to_sheet(visibleLocations.map((l, i) => {
             const key = `${l.lat},${l.lng}`;
             return {
                 'S.No': i + 1,
  ...
  const mapCenter: [number, number] = locations.length > 0 && currentIndex >= 0 && locations[currentIndex] ? [
    locations[currentIndex].lat,
    locations[currentIndex].lng
  ] : (locations.length > 0 && locations[0] ? [locations[0].lat, locations[0].lng] : [20.5937, 78.9629]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
      <div className="text-center mb-10">
        ...
        <h1 className="text-3xl font-bold text-slate-900 mb-4">Excel to 3D Map Bulk Tracker</h1>
        <p className="text-slate-600 max-w-2xl mx-auto">
          Upload an Excel/CSV file containing lat/long. Watch the sequence mapped out efficiently, filter by date/time, and export MS-Excel / PDF reports easily.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 mb-12">
        <div className="flex flex-col md:flex-row gap-6 mb-8">
          <div className="flex-1 w-full relative">
            ...
            />
            <div className={cn(
              "border-2 border-dashed rounded-xl flex items-center justify-center p-6 transition-colors h-32",
              file ? "border-blue-300 bg-blue-50" : "border-slate-300 bg-slate-50 hover:bg-slate-100"
            )}>
              <div className="flex flex-col items-center">
                <UploadCloud className={cn("w-8 h-8 mb-2", file ? "text-blue-500" : "text-slate-400")} />
                <p className="font-medium text-slate-700">{file ? file.name : "Upload CSV/Excel Data"}</p>
              </div>
            </div>
          </div>
          
          <div className="w-full md:w-1/3 flex flex-col justify-center">
             <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col justify-center h-32 text-center">
               <h4 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Locations Found</h4>
               <div className="flex flex-col items-center justify-center">
                  <span className="text-3xl font-bold text-slate-900">{loading ? totalParsedLocations : locations.length}</span>
                  {loading && loadingText && (
                     <div className="mt-2 flex items-center justify-center gap-2 text-blue-600">
                        <Loader2 className="w-4 h-4 animate-spin flex-shrink-0" />
                        <span className="text-xs font-medium whitespace-pre-line text-left leading-tight">{loadingText}</span>
                     </div>
                  )}
                  {loading && !loadingText && <Loader2 className="w-6 h-6 animate-spin text-blue-600 mt-2" />}
               </div>
             </div>
          </div>
        </div>
            ...
            <div className="lg:col-span-3 border border-slate-200 rounded-2xl overflow-hidden bg-slate-50 relative flex flex-col">
              <div className="px-4 py-4 bg-white border-b border-slate-200 flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="flex items-center gap-4">
                  <button 
                    ...
                    className={cn(
                      ...
                      animating ? "bg-red-500 hover:bg-red-600" : "bg-blue-600 hover:bg-blue-700"
                    )}
                  >
                    {animating ? <><Pause className="w-4 h-4 fill-current"/> Pause</> : <><Play className="w-4 h-4 fill-current"/> Play</>}
                  </button>
                  <div className="flex flex-col">
                    <span className="text-sm font-medium text-slate-500 uppercase tracking-wider">Point {Math.max(0, currentIndex + 1)} of {loading ? totalParsedLocations : locations.length}</span>
                    <span className="font-mono text-xs font-semibold text-slate-700">
                      {currentIndex >= 0 && locations[currentIndex] ? `${locations[currentIndex].lat.toFixed(6)}, ${locations[currentIndex].lng.toFixed(6)}` : "Ready to play"}
                    </span>
                    <div className="flex items-center gap-2 mt-2">
                       <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider min-w-[70px]">Delay: {(playbackSpeed/1000).toFixed(1)}s</span>
                       <input 
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                     <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Zoom: {mapZoom}</span>
                     <input 
                        type="range" 
                        min="1" max="22" 
                        value={mapZoom} 
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-green-700 text-white text-xs font-semibold rounded-lg hover:bg-green-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
             ...
             <div className="bg-slate-100 border-b border-slate-200 px-4 py-3">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-sm font-semibold text-slate-700">Filter by Date & Time Range:</span>
                </div>
                
                {fileDateBounds ? (
                  <div className="flex flex-wrap items-center gap-4">
                    <div className="flex flex-col">
                      <label className="text-xs text-slate-500 font-medium mb-1">Start Date/Time</label>
                      <select 
                        ...
                        onChange={(e) => {
                          ...
                          setDateRange(prev => ({ ...prev, start: isNaN(idx) ? fileDateBounds.min : idx }));
                          setCurrentIndex(-1);
                          setAnimating(false);
                          setShowFullPath(false);
                          setHasStartedPlaying(false);
                      ...
                      >
                        {availableDates.map(d => (
                           <option key={`start-${d.index}`} value={d.index}>{d.label}</option>
                        ))}
                      </select>
                    </div>
                    <div className="flex flex-col">
                      <label className="text-xs text-slate-500 font-medium mb-1">End Date/Time</label>
                      <select 
                        className="bg-white border border-slate-300 rounded px-2 py-1.5 text-sm text-slate-700 focus:ring-2 focus:ring-blue-500 max-w-[200px]"
                        value={dateRange.end !== null ? dateRange.end : ''}
                        onChange={(e) => {
                          const idx = Number(e.target.value);
                          setDateRange(prev => ({ ...prev, end: isNaN(idx) ? fileDateBounds.max : idx }));
                          setCurrentIndex(-1);
                          setAnimating(false);
                          setShowFullPath(false);
                          setHasStartedPlaying(false);
                      ...
                      >
                        {availableDates.map(d => (
                           <option key={`end-${d.index}`} value={d.index}>{d.label}</option>
                        ))}
                      </select>
                    </div>
                  </div>
                ) : (
                  <div className="text-sm text-slate-500">No date/time found in the file to filter.</div>
                )}
             </div>

             <div id="map-container-for-pdf" className="h-[600px] w-full relative bg-[#e5e3df] z-0 overflow-hidden">
                 ...
                 >
                        ...
                        <LayerGroup>
                          {visibleLocations.map((l, i) => (
                            <CircleMarker
                              ...
                              center={[l.lat, l.lng]}
                              radius={25}
                              pathOptions={{ color: 'transparent', fillColor: '#22c55e', fillOpacity: 0.15 }}
                            />
                          ))}
                        </LayerGroup>
                      </LayersControl.Overlay>
                    ...
                    {visibleLocations.map((l, i) => {
                      const key = `${l.lat},${l.lng}`;
                      const addr = addressCache[key] || 'पता खोजा जा रहा है... (Finding address...)';
                      return (
                      <CircleMarker 
                        key={i} 
                        center={[l.lat, l.lng]} 
                        ...
                        eventHandlers={{
                          mouseover: () => {
                             if (!addressCache[key] && !pendingLookups.current.has(key)) {
                      ...
                      >
                         <Tooltip direction="top" offset={[0, -5]} opacity={1}>
                            <div className="flex flex-col max-w-[200px] whitespace-normal">
                               {l.datetime && l.datetime !== '-' && <span className="font-semibold text-sm mb-1">{l.datetime}</span>}
                               <span className="text-xs text-slate-600 mb-1 font-mono">{l.lat.toFixed(6)}, {l.lng.toFixed(6)}</span>
                               <span className="text-xs font-medium text-slate-800 leading-tight">{addr}</span>
                            </div>
                         </Tooltip>
                         <Popup offset={[0, -5]}>
                            <div className="flex flex-col max-w-[200px] whitespace-normal">
                    ...
                    {hasStartedPlaying && currentIndex >= 0 && locations.length > 0 && locations[currentIndex] && (
                      <Marker position={interpolatedPos && animating ? interpolatedPos : mapCenter} icon={activeIcon} zIndexOffset={100}>
                         <Tooltip direction="top" offset={[0, -20]} opacity={1}>
                            <div className="flex flex-col max-w-[200px] whitespace-normal">
                               {locations[currentIndex].datetime && locations[currentIndex].datetime !== '-' && <span className="font-semibold text-sm mb-1">{locations[currentIndex].datetime}</span>}
                               <span className="text-xs text-slate-600 mb-1 font-mono">{locations[currentIndex].lat.toFixed(6)}, {locations[currentIndex].lng.toFixed(6)}</span>
                               <span className="text-xs font-medium text-slate-800 leading-tight">{currentAddress}</span>
                            </div>
                         </Tooltip>
                         <Popup offset={[0, -20]}>
                            <div className="flex flex-col max-w-[200px] whitespace-normal">
            ...
            <div className="lg:col-span-1 border border-slate-200 rounded-2xl bg-white shadow-sm flex flex-col h-full max-h-[700px] overflow-hidden">
               ...
               <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {upcomingLocations.length === 0 ? (
                     <div className="text-sm text-slate-500 text-center py-8">No route points to display.</div>
                  ) : (
                     <div className="relative">
                        ...
                        {upcomingLocations.map((loc, idx) => {
                           ...
                           return (
                             <div key={`${loc.lat}-${loc.lng}-${idx}`} className={cn(
                               "relative z-10 flex gap-4 p-3 rounded-lg border bg-white transition-colors",
                               isCurrent ? "border-blue-300 shadow-sm ring-1 ring-blue-100" : "border-slate-100 opacity-80",
                               "mb-3 last:mb-0"
                             )}>
                                <div className={cn(
                                  "w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 font-bold text-xs ring-4 ring-white",
                                  isCurrent ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500 border border-slate-200"
                                )}>
                                  {loc.index + 1}
                                </div>
                                <div className="flex flex-col min-w-0">
                                   <div className="flex items-center gap-2 mb-0.5">
                                      <span className={cn(
                                        ...
                                        {loc.datetime && loc.datetime !== '-' ? loc.datetime : `Point ${loc.index + 1}`}
                                      </span>
                                      {isCurrent && (
                                        <span className="px-1.5 py-0.5 text-[10px] font-bold tracking-wider uppercase bg-blue-100 text-blue-700 rounded">Active</span>
                                      )}
                                   </div>
        ...
        <div className="space-y-6">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1">Upload an Excel or CSV file containing latitude and longitude coordinates. They can be in separate columns or a single comma-separated column. Auto-extracts Date and Time to plot on timeline.</p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1">Pick a specific point in the dropdown above the map. You can only pick dates available in the uploaded file.</p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1">Click 'Play Target'. The map will intelligently animate your travel, adapting icons (Car/Bike/Walk) based on calculated speed and smoothly flying to each destination.</p>
            </div>
          </div>
        </div>
      </div>
