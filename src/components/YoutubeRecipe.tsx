...
export default function YoutubeRecipe() {
  ...
  const downloadPdf = () => {
     ...
     try {
       const doc = new jsPDF();
       const totalCost = result.ingredients.reduce((total: number, item: any) => total + (Number(item.estimatedPrice) || 0), 0);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="text-center space-y-4">
        ...
        <h1 className="text-3xl font-bold text-slate-800">YouTube Recipe to Grocery Checklist</h1>
        <p className="text-slate-500 max-w-2xl mx-auto">Found a delicious recipe on YouTube? Paste the link and we will extract a complete grocery shopping checklist with estimated market prices so you don't forget anything.</p>
      </div>
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        <div className="mb-6 grid grid-cols-1 md:grid-cols-4 gap-4">
           <div className="md:col-span-3">
             ...
             <input type="url" value={url} onChange={(e)=>setUrl(e.target.value)} placeholder="https://www.youtube.com/watch?v=..." className="w-full border-slate-300 rounded-lg shadow-sm focus:ring-red-500 focus:border-red-500 p-3 border" />
           </div>
           <div>
             <label className="block text-sm font-medium text-slate-700 mb-2">Output Language</label>
             <select value={language} onChange={(e)=>setLanguage(e.target.value)} className="w-full border-slate-300 rounded-lg shadow-sm focus:ring-red-500 focus:border-red-500 p-3 border bg-white">
      ...
      {result && result.ingredients && (
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 border-b border-slate-100 pb-4">
            <div>
              <h3 className="text-xl font-bold text-slate-800">{result.recipeName}</h3>
              <p className="text-sm text-slate-500">Checklist Ready for Shopping</p>
            </div>
            ...
            <table className="w-full text-left border-collapse">
               <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                     ...
                     <th className="p-3 text-sm font-semibold text-slate-600">Shopping Item</th>
                     <th className="p-3 text-sm font-semibold text-slate-600">Quantity</th>
                     <th className="p-3 text-sm font-semibold text-slate-600 border-l border-slate-200">Est. Price (₹)</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-slate-100">
                 {result.ingredients.map((item: any, idx: number) => (
                    <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                       ...
                       <td className="p-3 font-medium text-slate-800 whitespace-pre-wrap">{item.item}</td>
                       <td className="p-3 text-slate-600 whitespace-pre-wrap">{item.quantity}</td>
               ...
               <tfoot>
                 <tr className="bg-slate-50 border-t-2 border-slate-200 font-bold">
                    <td colSpan={3} className="p-4 text-right text-slate-800">Total Estimated Cost:</td>
                    <td className="p-4 text-slate-800 font-mono">
                      ₹{result.ingredients.reduce((total: number, item: any) => total + (Number(item.estimatedPrice) || 0), 0)}
                    </td>
                 </tr>
               </tfoot>
            </table>
        ...
        <div className="space-y-6">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1">Copy the URL of the cooking recipe video from YouTube.</p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1">Paste the link and our AI will watch/read the video details to extract every ingredient mentioned, translating it cleanly.</p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              <h4 className="font-semibold text-slate-900">Download Shopping List</h4>
              <p className="text-slate-600 text-sm mt-1">Download the neat PDF checklist onto your phone. It includes checkboxes and estimated market prices so you can tick them off while shopping at the grocery store.</p>
            </div>
          </div>
        </div>
      </div>
