...
import { FileSpreadsheet, UploadCloud, Search, Download, FileType2, Loader2, Link as LinkIcon } from 'lucide-react';
...
export default function CommonDataFinder() {
    ...
    const [commonMatches, setCommonMatches] = useState<CommonMatch[] | null>(null);
    ...
    try {
      for (const file of selected) {
        ...
        if (file.name.endsWith('.csv')) {
          ...
          rawData = XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: '' }) as any[][];
        } else {
        ...
        const headers = rawData[headerRowIdx] ? rawData[headerRowIdx].map(c => String(c).trim()) : [];
        ...
        if (cleanHeaders.length > 0) {
           parsedData.push({ fileName: file.name, data: dataObjects, headers: cleanHeaders.filter(h => h && !h.startsWith('Column_')) });
        }
      }
    ...
    setTimeout(() => {
      ...
      fileDataArray[0].data.forEach(row => {
        ...
        if (val) {
          ...
          baseMap.get(key)!.push({ fileName: fileDataArray[0].fileName, rowData: row });
        }
      });
      ...
      for (let i = 1; i < fileDataArray.length; i++) {
        ...
        const currentFileMap = new Map<string, { fileName: string; rowData: any }[]>();
        ...
        fileDataArray[i].data.forEach(row => {
          ...
          if (val) {
            ...
            currentFileMap.get(key)!.push({ fileName: currentFileName, rowData: row });
          }
        });
  ...
  const handleDownloadCSV = () => {
    ...
    const flatData = commonMatches.map(match => ({ [headerName]: match.matchValue, 'Count': match.count }));
  ...
  const handleDownloadExcel = () => {
    ...
    const headerName = selectedColumns[0] || 'Common Value';
    const excelData = commonMatches.map(match => ({ [headerName]: match.matchValue, 'Count': match.count }));
  ...
  const handleDownloadPDF = () => {
    ...
    const body: any[][] = commonMatches.map((match, idx) => [idx + 1, match.matchValue, match.count]);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="text-center mb-10">
        ...
        <h1 className="text-3xl font-bold text-slate-900 mb-4">Common Data Finder</h1>
        <p className="text-slate-600 max-w-2xl mx-auto">
          Upload 2 or more CSV/Excel files and instantly find common data across them. It groups matching details so you can see full rows from both files together.
        </p>
      </div>

      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 mb-12">
        <div className="border-2 border-dashed border-slate-300 bg-slate-50 hover:bg-slate-100 cursor-pointer rounded-xl flex flex-col items-center justify-center p-8 transition-colors relative mb-8">
          <input 
            type="file" 
            multiple
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-1">Select Multiple Files</h3>
          <p className="text-sm text-slate-500">Hold Ctrl/Cmd to select 2 or more files (CSV or Excel)</p>
        </div>

        {fileDataArray.length > 0 && (
          <div className="mb-8">
             <h4 className="font-semibold text-slate-700 mb-4 text-lg">Select Columns to Match:</h4>
                 ...
                 return (
                   <div key={index} className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                     <div className="flex items-center text-slate-700 font-medium mb-3">
                       ...
                       <span className="truncate" title={fileInfo.fileName}>{fileInfo.fileName}</span>
                     </div>
                     <select 
             ...
             >
               {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><Search className="w-4 h-4 mr-2" /> Find Common Data</>}
             </button>
          </div>
        )}
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden max-h-[600px] overflow-y-auto w-full md:w-1/2">
                <table className="w-full text-left bg-white text-sm">
                  <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
                     <tr>
                        ...
                        <th className="px-6 py-3 font-semibold text-slate-700">{selectedColumns[0] || 'Common Value'}</th>
                        <th className="px-6 py-3 font-semibold text-slate-700 text-right w-24">Count</th>
                     </tr>
                  </thead>
                ...
                {commonMatches.length > 100 && (
                  <div className="p-3 text-center text-sm text-slate-500 bg-slate-50 border-t border-slate-200">
                    Showing first 100 common matches. Please download to view all {commonMatches.length} matches.
                  </div>
                )}
              </div>
            ) : (
               <div className="bg-slate-50 border border-slate-200 rounded-xl p-8 text-center text-slate-500">
                 No common data found. (Make sure you selected the correct matching columns).
               </div>
            )}
          </div>
        )}
        ...
        <div className="space-y-6">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1">Select 2 or more files (CSV or Excel). Hold Ctrl/Cmd to select multiple files at the same time.</p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1">From the dropdowns below each file, select the column you want to compare (like "Mobile Number" from file 1 and "Phone" from file 2).</p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              <h4 className="font-semibold text-slate-900">Get Results & Download</h4>
              <p className="text-slate-600 text-sm mt-1">Click 'Find Common Data'. You will see a list of only those numbers or data points that are present in all the selected files. You can download this list as CSV, Excel, or PDF.</p>
            </div>
          </div>
        </div>
      </div>
