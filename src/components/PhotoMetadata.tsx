...
export default function PhotoMetadata() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const img = new Image();
      const resolutionPromise = new Promise<{ width: number; height: number }>(
        (resolve) => {
      ...
      const output = await exifr.parse(selected, {
        gps: true,
        exif: true,
        tiff: true,
      });
          ...
          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
            {loading ? (
              ...
              <div className="h-full flex items-center justify-center text-slate-400 text-center text-sm">
                Upload a photo to view its details, time, and location
                information.
              </div>
            ) : (
                ...
                {Object.keys(metadata).length === 0 ? (
                  <div className="flex flex-col items-center justify-center text-slate-500 text-center p-6 bg-slate-50 rounded-xl">
                    <p className="font-medium mb-2">
                      No Hidden EXIF Data Found
                    </p>
                    <p className="text-sm text-slate-400">
                      Advanced EXIF metadata (camera model, GPS) has been
                      stripped or was never present. This often happens if the
                      photo was sent via WhatsApp, Facebook, or downloaded from
                      the web.
                    </p>
                  </div>
                ) : (
                  <>
                      ...
                      <div className="bg-slate-50 border border-slate-200 rounded-xl divide-y divide-slate-100">
                        ...
                        {metadata.ExposureTime && (
                          <div className="p-3 bg-slate-50 flex justify-between items-center group">
                            ...
                            <span className="font-mono text-xs text-slate-900">
                              1/{Math.round(1 / metadata.ExposureTime)} sec
                            </span>
                          </div>
                        )}
                        {metadata.ISO && (
                    ...
                    {metadata.latitude && metadata.longitude && (
                      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                        <h4 className="font-bold text-blue-900 flex items-center gap-2 mb-3">
                          <MapPin className="w-5 h-5 text-blue-600" /> GPS
                          Location Found!
                        </h4>
                    ...
                    <div>
                      ...
                      <div className="bg-slate-900 text-blue-400 font-mono text-xs p-4 rounded-xl overflow-x-auto">
        ...
        <div className="space-y-6">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1">
                Click the Select Photo button and pick an original image file.
                Remember, images sent via WhatsApp or downloaded from the
                internet usually have their EXIF metadata removed to protect
                privacy.
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1">
                The application reads the image properties entirely in your
                browser without uploading to a server, ensuring your data is
                kept secure.
              </p>
            </div>
          </div>
          <div className="flex gap-4">
            ...
            <div>
              <h4 className="font-semibold text-slate-900">
                Check Location and Camera
              </h4>
              <p className="text-slate-600 text-sm mt-1">
                If available, it will display the date taken, the camera/phone
                model, and even provide a Google Maps link to the exact
                coordinates where the picture was snapped.
              </p>
            </div>
          </div>
        </div>
